...
class Keyframe:
    ...
    def __init__(
        ...
        Args:
            image: 输入图像 [3, H, W]
            info: 关键帧元信息（是否测试帧、名称、COLMAP位姿等）
            desc_kpts: 描述的关键点（包含2D坐标、描述子、3D点等）
            Rt: 相机到世界变换矩阵 [4, 4]
            index: 关键帧索引
            f: 焦距（像素）
            feat_extractor: 密集特征提取器
            depth_estimator: 单目深度估计器
            triangulator: 三角化器
            args: 训练参数
            inference_mode: 是否为推理模式
        """
        # ========== 图像数据初始化 ==========
        self.image_pyr = [image]  # 图像金字塔（多分辨率）
        
        if not inference_mode:  # 训练模式下提取深度和特征
            # 【场景表示模块】提取密集特征图（用于引导MVS）
            ...
            self.depth_loss_weight = args.depth_loss_weight_init  # 深度损失初始权重
            self.depth_loss_weight_decay = args.depth_loss_weight_decay  # 深度损失衰减因子
            ...
            self.pyr_lvl = args.pyr_levels - 1  # 当前使用的金字塔层级
        ...
        self.index = index
        self.idepth_pyr = None  # 逆深度金字塔（延迟初始化）
        self.latest_invdepth = None  # 最新渲染的逆深度（用于三角化）
    ...
    def update_3dpts(self, all_keyframes: list[Keyframe]):
        ...
        通过两种方式更新3D点：
        1. 从最新渲染深度采样（如果渲染深度与单目深度一致）
        2. 通过三角化（使用历史关键帧的匹配点）
        
        Args:
            all_keyframes: 所有关键帧列表（用于三角化）
        """
