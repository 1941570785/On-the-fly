    ...
    def _export_new_keyframes(scene_model: SceneModel):
        ...
        for kid, kf in enumerate(scene_model.keyframes):
            ...
            header = [
                ...
                "depth_align_error_median",
                "depth_align_inlier_ratio",
                "num_matches_before_pnp",
                "num_pnp_inliers",
                "pnp_inlier_ratio",
                ...
                "spawn_mvs_conf_mean",
                "spawn_dynamic_mask_mean",
                "spawn_dynamic_kept_mean",
                "dynamic_mask_mean",
                "dynamic_suppressed_ratio",
                "replay_score",
                "num_keyframes",
                "num_anchors",
                "num_gaussians",
            ]
            diag = kf.info.get("diagnostics", {})
            row = [
                time.time(),
                int(kid),
                int(diag.get("frame_id", kid)),
                kf.info.get("name", ""),
                ...
                float(ssim_val),
                float(lpips_val),
                float(diag.get("render_l1", float("nan"))),
                float(diag.get("render_dssim", float("nan"))),
                float(diag.get("depth_loss", float("nan"))),
                float(diag.get("depth_align_error_mean", float("nan"))),
                float(diag.get("depth_align_error_median", float("nan"))),
                float(diag.get("depth_align_inlier_ratio", float("nan"))),
                float(diag.get("num_matches_before_pnp", float("nan"))),
                float(diag.get("num_pnp_inliers", float("nan"))),
                float(diag.get("pnp_inlier_ratio", float("nan"))),
                float(diag.get("miniba_final_residual", float("nan"))),
                float(diag.get("pose_confidence", float("nan"))),
                float(diag.get("spawn_candidates", float("nan"))),
                float(diag.get("spawn_selected", float("nan"))),
                float(diag.get("spawn_valid_depth", float("nan"))),
                float(diag.get("spawn_uncertainty_mean", float("nan"))),
                float(diag.get("spawn_mvs_conf_mean", float("nan"))),
                float(diag.get("spawn_dynamic_mask_mean", float("nan"))),
                float(diag.get("spawn_dynamic_kept_mean", float("nan"))),
                float(diag.get("dynamic_mask_mean", float("nan"))),
                float(diag.get("dynamic_suppressed_ratio", float("nan"))),
                float(diag.get("replay_score", float("nan"))),
