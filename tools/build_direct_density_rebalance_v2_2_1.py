#!/usr/bin/env python3
"""PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2_2_1 — gap-tail refinement post-run."""
...
def load_v221_events(trace: dict[str, Any], run_dir: Path) -> list[dict[str, Any]]:
    return trace.get("direct_density_control_v2_2_1_events", []) or read_csv(
        run_dir / "direct_density_control_v2_2_1_trace.csv"
    )
...
def analyze_run(label: str, run_dir: Path, nframes: int) -> dict[str, Any]:
    ...
    v2 = load_v221_events(trace, run_dir)
    recovery_kf = sum(
        1
...
def export_run_dir(run_dir: Path, label: str, nframes: int) -> None:
    trace = load_trace(run_dir)
    v2 = trace.get("direct_density_control_v2_2_1_events", []) or []
    ...
    if v2:
        write_csv(run_dir / "direct_density_control_v2_2_1_trace.csv", v2)
    for key, fname in [
...
def baseline_guard_audit(model_dir: Path, out: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "direct_density_control_v2_2_1_events": len(v221),
...
def post_run(out: Path) -> None:
    runs = {
        "direct_density_v2_2_1_short500": (out / "direct_density_v2_2_1_short500", 500),
        "direct_density_v2_2_1_short800": (out / "direct_density_v2_2_1_short800", 800),
        "direct_density_v2_2_1_short1000": (out / "direct_density_v2_2_1_short1000", 1000),
    }
    analyzed = {}
    ...
    v22_500 = (
        analyze_run("v22_short500", V22 / "direct_density_v2_2_short500", 500)
        if (V22 / "direct_density_v2_2_short500").exists()
        else {}
    )
    v22_800 = (
        analyze_run("v22_short800", V22 / "direct_density_v2_2_short800", 800)
        if (V22 / "direct_density_v2_2_short800").exists()
        else {}
    )
    v22_1000 = (
        analyze_run("v22_short1000", V22 / "direct_density_v2_2_short1000", 1000)
        if (V22 / "direct_density_v2_2_short1000").exists()
        else {}
    )

    rows = list(analyzed.values())
    write_csv(out / "direct_density_v2_2_1_short_comparison.csv", rows)
    write_json(out / "direct_density_v2_2_1_short_comparison.json", {"rows": rows})

    r800 = analyzed.get("direct_density_v2_2_1_short800")
    r1000 = analyzed.get("direct_density_v2_2_1_short1000", {})
    ...
    effect = {
        "v22_short500_density": v22_500.get("keyframes_per_100"),
        "v22_short800_density": v22_800.get("keyframes_per_100"),
        "v22_short1000_density": v22_1000.get("keyframes_per_100"),
        "v22_short500_gap_p90": v22_500.get("gap_p90"),
        "v22_short800_gap_p90": v22_800.get("gap_p90"),
        "v2_2_1_short500_density": analyzed.get("direct_density_v2_2_1_short500", {}).get("keyframes_per_100"),
        "v2_2_1_short800_density": analyzed.get("direct_density_v2_2_1_short800", {}).get("keyframes_per_100"),
        "v2_2_1_short1000_density": r1000.get("keyframes_per_100"),
        "v22_short500_gap_max": v22_500.get("gap_max"),
        "v2_2_1_short500_gap_max": analyzed.get("direct_density_v2_2_1_short500", {}).get("gap_max"),
        "v22_short800_gap_p90": v22_800.get("gap_p90"),
        "v2_2_1_short800_gap_p90": analyzed.get("direct_density_v2_2_1_short800", {}).get("gap_p90"),
    }
    write_json(out / "direct_density_v2_2_1_effect_summary.json", effect)
    ...
    ready = {
        **gate1000,
        ...
        "gate_version": "direct_density_v2_2_1",
        "per_run": per_run,
    }
    write_json(out / "ready_for_full_gate_after_direct_density_v2_2_1.json", ready)

    r500 = analyzed.get("direct_density_v2_2_1_short500", {})
    lines = [
        "# PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2_2_1",
        "",
        "## 1. short500 under-density 是否修复？",
        f"- v2.2={effect.get('v22_short500_density')} v2.2.1={effect.get('v2_2_1_short500_density')} (target 25-50)",
        "",
        "## 2. short800/1000 是否避免 starvation？",
        f"- v2.2 800/1000={effect.get('v22_short800_density')}/{effect.get('v22_short1000_density')}",
        f"- v2.2.1 800/1000={effect.get('v2_2_1_short800_density')}/{effect.get('v2_2_1_short1000_density')} (target 25-45)",
        "",
        "## 3. gap max 是否 <=20？",
        f"- v2.2 gap 500 max/p90={effect.get('v22_short500_gap_max')}/{effect.get('v22_short500_gap_p90')}",
        f"- v2.2.1 gap 500 max={effect.get('v2_2_1_short500_gap_max')}",
        f"- v2.2 gap 800 p90={effect.get('v22_short800_gap_p90')} v2.2.1={effect.get('v2_2_1_short800_gap_p90')}",
        "",
    ...
    write_md(out / "paper_aligned_direct_density_rebalance_v2_2_1_report.md", lines)
