...
def main() -> int:
    ...
    mode = str(trace.get("mode", "off" if not trace_present else ""))
    control_mode = str(trace.get("recovery_commit_control_mode", "off" if not trace_present else ""))
    ...
    write_json(output_dir / "strict_v3_baseline_guard_engine_audit.json", engine)
    write_json(
        output_dir / "strict_v3_baseline_guard_trace_audit.json",
        {
            "mode": mode,
            "recovery_commit_control_mode": control_mode,
            ...
            "guard_expectation_off_mode": bool(mode == "off"),
        },
    )
    ...
    report_lines = [
        "# strict_v3 baseline guard report",
        "",
        f"- train_returncode: {train_code}",
        f"- risk_admission_mode: {mode}",
        f"- recovery_commit_control_mode: {control_mode}",
        f"- control_events: {len(control_events)}",
        f"- true_recovery_events: {len(true_events)}",
    ...
    (output_dir / "strict_v3_baseline_guard_report.md").write_text(
        "\n".join(report_lines).rstrip() + "\n", encoding="utf-8"
    )
    write_json(
        output_dir / "ready_for_strict_v3_short_run_after_guard.json",
        {
