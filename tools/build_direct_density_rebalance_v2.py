#!/usr/bin/env python3
"""PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2 — post-run export and full gate."""
...
def analyze_run(label: str, run_dir: Path, nframes: int) -> dict[str, Any]:
    ...
    v2 = trace.get("direct_density_control_v2_events", []) or read_csv(
        run_dir / "direct_density_control_v2_trace.csv"
    )
    recovery_kf = sum(
        1
...
def export_run_dir(run_dir: Path, label: str, nframes: int) -> None:
    trace = load_trace(run_dir)
    v2 = trace.get("direct_density_control_v2_events", []) or []
    ...
    if v2:
        write_csv(run_dir / "direct_density_control_v2_trace.csv", v2)
    for key, fname in [
...
def baseline_guard_audit(model_dir: Path, terminal: Path, out: Path) -> dict[str, Any]:
    ...
    v2ev = trace.get("direct_density_control_v2_events", []) or []
    ...
    payload = {
        ...
        "direct_density_control_v2_events": len(v2ev),
...
def post_run(out: Path) -> None:
    runs = {
        "direct_density_v2_short500": (out / "direct_density_v2_short500", 500),
        "direct_density_v2_short800": (out / "direct_density_v2_short800", 800),
        "direct_density_v2_short1000": (out / "direct_density_v2_short1000", 1000),
    }
    analyzed = {}
    ...
    write_csv(out / "direct_density_v2_short_comparison.csv", rows)
    write_json(out / "direct_density_v2_short_comparison.json", {"rows": rows})

    r800 = analyzed.get("direct_density_v2_short800")
    r1000 = analyzed.get("direct_density_v2_short1000", {})
    gate1000 = eval_full_gate(r1000, r800) if r1000 else {}

    effect = {
        "v1_short500_density": v1_500.get("keyframes_per_100"),
        "v2_short500_density": analyzed.get("direct_density_v2_short500", {}).get("keyframes_per_100"),
        "v1_short1000_density": analyze_run("v1", V1 / "direct_density_short1000", 1000).get("keyframes_per_100")
        if (V1 / "direct_density_short1000").exists()
        else None,
        "v2_short1000_density": r1000.get("keyframes_per_100"),
        "budget_exhausted_500": analyzed.get("direct_density_v2_short500", {}).get("high_novelty_budget_exhausted"),
        "hold_redundant_blocked_1000": r1000.get("hold_redundant_blocked"),
    }
    write_json(out / "direct_density_v2_effect_summary.json", effect)

    ready = {
        **gate1000,
        "recommend_full_run": bool(gate1000.get("ready_for_full_forest1")),
        "keep_RVQ_tau_frozen": True,
        "gate_version": "direct_density_v2",
        "per_run": {k: eval_full_gate(v, r800) for k, v in analyzed.items()},
    }
    write_json(out / "ready_for_full_gate_after_direct_density_v2.json", ready)

    lines = [
        "# PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2",
        "",
        "## 1. short500 过密是否解决？",
        f"- v1={effect.get('v1_short500_density')} v2={effect.get('v2_short500_density')} (target <=50)",
        "",
        "## 2. short1000 starvation？",
        f"- v1={effect.get('v1_short1000_density')} v2={effect.get('v2_short1000_density')} (target >=25, kf>=216)",
        "",
        ...
        f"- prev_desc_updated_on_hold@500: {analyzed.get('direct_density_v2_short500', {}).get('prev_desc_updated_on_hold', 0)}",
        "",
    ...
    write_md(out / "paper_aligned_direct_density_rebalance_v2_report.md", lines)
