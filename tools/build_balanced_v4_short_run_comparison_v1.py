...
def main() -> int:
    ...
    kf_growth = kf500 - kf300

    rows = [
        {"run": "live_short_300", **r300},
        {"run": "live_short_500", **r500},
    ]
    write_csv(short / "balanced_v4_short_run_comparison.csv", rows)
    ...
    comp = {
        "live_short_300": r300,
        "live_short_500": r500,
        "keyframe_growth_300_to_500": kf_growth,
        "ready_for_balanced_v4_full_run": ready,
    }
    write_json(short / "balanced_v4_short_run_comparison.json", comp)
    write_json(short / "ready_for_balanced_v4_full_run.json", {"ready_for_balanced_v4_full_run": ready})
    (short / "paper_aligned_balanced_v4_short_run_report.md").write_text(
        "\n".join(
            [
                "# balanced_v4 short run report",
                "",
                f"- live_short_300 keyframes/density: {kf300}/{r300.get('keyframes_per_100_frames', None)}",
                f"- live_short_500 keyframes/density: {kf500}/{r500.get('keyframes_per_100_frames', None)}",
                f"- keyframe_growth_300_to_500: {kf_growth}",
                f"- gap p90/p95/max @500: {r500.get('main_chain_gap_p90', None)}/{r500.get('main_chain_gap_p95', None)}/{r500.get('main_chain_gap_max', None)}",
                f"- ready_for_balanced_v4_full_run: {ready}",
            ]
        )
        + "\n",
        encoding="utf-8",
