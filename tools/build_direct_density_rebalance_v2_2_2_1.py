#!/usr/bin/env python3
"""PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2_2_2_1 — post-500 pre-gap + hard bypass post-run."""
...
def load_v2221_events(trace: dict[str, Any], run_dir: Path) -> list[dict[str, Any]]:
    return trace.get("direct_density_control_v2_2_2_1_events", []) or read_csv(
        run_dir / "direct_density_control_v2_2_2_1_trace.csv"
    )
...
def analyze_run(label: str, run_dir: Path, nframes: int) -> dict[str, Any]:
    ...
    v2 = load_v2221_events(trace, run_dir)
    recovery_kf = sum(
        1
    ...
    return {
        ...
        "post500_pre_gap_count": sum(
            1
            for e in v2
            if to_bool(e.get("post500_pre_gap_rescue_finalized"))
            or str(e.get("direct_finalization_decision", ""))
            == "finalize_post500_pre_gap_rescue_v2_2_2_1"
        ),
        "hard_rescue_v2221_count": sum(
            1
            for e in v2
            if to_bool(e.get("hard_gap_rescue_finalized"))
            or str(e.get("direct_finalization_decision", ""))
            == "finalize_hard_gap_rescue_v2_2_2_1"
        ),
        "hard_budget_bypass_count": sum(
            1 for e in v2 if to_bool(e.get("hard_gap_soft_budget_bypassed"))
        ),
...
def export_run_dir(run_dir: Path, label: str, nframes: int) -> None:
    trace = load_trace(run_dir)
    v2 = trace.get("direct_density_control_v2_2_2_1_events", []) or []
    ...
    if v2:
        write_csv(run_dir / "direct_density_control_v2_2_2_1_trace.csv", v2)
    for key, fname in [
    ...
    (run_dir / "report.md").write_text(
        ...
        f"- post500_pre_gap={summary.get('post500_pre_gap_count')}\n",
        encoding="utf-8",
    )
...
def baseline_guard_audit(model_dir: Path, out: Path) -> dict[str, Any]:
    ...
    v222 = trace.get("direct_density_control_v2_2_2_1_events", []) or []
    ...
    payload = {
        ...
        "direct_density_control_v2_2_2_1_events": len(v222),
...
def frame_level_gap_report(out: Path, r500: dict[str, Any]) -> None:
    ...
    trace = load_v2221_events(load_trace(out / "direct_density_v2_2_2_1_short500"), out / "direct_density_v2_2_2_1_short500")
    hold_rows = [
        e
        for e in trace
    ...
    preempt = [
        ...
        if 430 <= int(e.get("frame_id", 0)) <= 452
        and (
            to_bool(e.get("finalize_gap_tail_preemptive_v2_2_2_1"))
            or str(e.get("direct_finalization_decision", ""))
            == "finalize_gap_tail_preemptive_v2_2_2_1"
        )
    ]
    payload = {
...
def short800_1000_gate(out: Path) -> int:
    ok = True
    metrics = {}
    for key, nf, dlo, dhi in (
        ("short800", 800, 25, 45),
        ("short1000", 1000, 25, 45),
    ):
        run_dir = out / f"direct_density_v2_2_2_1_{key}"
        if not run_dir.exists():
            return 1
        export_run_dir(run_dir, f"direct_density_v2_2_2_1_{key}", nf)
        r = analyze_run(f"direct_density_v2_2_2_1_{key}", run_dir, nf)
        metrics[key] = r
        run_ok = bool(
            r["train_returncode"] == 0
            and dlo <= r["keyframes_per_100"] <= dhi
            and r["gap_p90"] <= 5
            and r["gap_p95"] <= 7
            and r["gap_max"] <= 20
            and (r["final_keyframe_count"] >= 216 if nf >= 1000 else True)
        )
        ok = ok and run_ok
    write_json(out / "short800_1000_gate_check.json", {"passed": ok, "metrics": metrics})
    return 0 if ok else 1


def short500_gate(out: Path) -> int:
    run_dir = out / "direct_density_v2_2_2_1_short500"
    if not run_dir.exists():
        return 1
    export_run_dir(run_dir, "direct_density_v2_2_2_1_short500", 500)
    r = analyze_run("direct_density_v2_2_2_1_short500", run_dir, 500)
...
def post_run(out: Path) -> None:
    runs = {
        "direct_density_v2_2_2_1_short500": (out / "direct_density_v2_2_2_1_short500", 500),
        "direct_density_v2_2_2_1_short800": (out / "direct_density_v2_2_2_1_short800", 800),
        "direct_density_v2_2_2_1_short1000": (out / "direct_density_v2_2_2_1_short1000", 1000),
    }
    ...
    v222_500 = (
        analyze_run("v222_short500", V222 / "direct_density_v2_2_2_short500", 500)
        if (V222 / "direct_density_v2_2_2_short500").exists()
        else {}
    )
    v222_800 = (
        analyze_run("v222_short800", V222 / "direct_density_v2_2_2_short800", 800)
        if (V222 / "direct_density_v2_2_2_short800").exists()
        else {}
    )

    effect = {
        "v222_short500_density": v222_500.get("keyframes_per_100"),
        "v222_short500_gap_max": v222_500.get("gap_max"),
        "v222_short800_gap_p90": v222_800.get("gap_p90"),
        "v222_short1000_gap_max": (
            analyze_run("v222_short1000", V222 / "direct_density_v2_2_2_short1000", 1000).get("gap_max")
            if (V222 / "direct_density_v2_2_2_short1000").exists()
            else None
        ),
        "v2_2_2_1_short500_density": analyzed.get("direct_density_v2_2_2_1_short500", {}).get("keyframes_per_100"),
        "v2_2_2_1_short800_density": analyzed.get("direct_density_v2_2_2_1_short800", {}).get("keyframes_per_100"),
        "v2_2_2_1_short1000_density": analyzed.get("direct_density_v2_2_2_1_short1000", {}).get("keyframes_per_100"),
        "v2_2_2_1_short500_gap_max": analyzed.get("direct_density_v2_2_2_1_short500", {}).get("gap_max"),
        "v2_2_2_1_short800_gap_p90": analyzed.get("direct_density_v2_2_2_1_short800", {}).get("gap_p90"),
        "v2_2_2_1_short1000_gap_max": analyzed.get("direct_density_v2_2_2_1_short1000", {}).get("gap_max"),
        "v2_2_2_1_short500_post500_pre_gap": analyzed.get("direct_density_v2_2_2_1_short500", {}).get(
            "post500_pre_gap_count"
        ),
    }
    rows = list(analyzed.values())
    write_csv(out / "direct_density_v2_2_2_1_short_comparison.csv", rows)
    write_json(out / "direct_density_v2_2_2_1_short_comparison.json", {"rows": rows})

    r800 = analyzed.get("direct_density_v2_2_2_1_short800")
    r1000 = analyzed.get("direct_density_v2_2_2_1_short1000", {})
    ...
    write_json(out / "direct_density_v2_2_2_1_effect_summary.json", effect)

    r500 = analyzed.get("direct_density_v2_2_2_1_short500", {})
    ...
    ready = {
        ...
        "gate_version": "direct_density_v2_2_2_1",
        "per_run": per_run,
    }
    write_json(out / "ready_for_full_gate_after_direct_density_v2_2_2_1.json", ready)

    lines = [
        "# PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2_2_2",
        "",
        "## 1. short500 430-452 gap tail 是否修复？",
        f"- v2.2.1 gap_max={effect.get('v221_short500_gap_max')} v2.2.2={effect.get('v2_2_2_1_short500_gap_max')}",
        f"- preemptive rescues={effect.get('v2_2_2_1_short500_preemptive')}",
        "",
    ...
    write_md(out / "paper_aligned_direct_density_rebalance_v2_2_2_1_report.md", lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--phase",
        choices=["baseline_guard", "short800_1000_gate", "short500_gate", "post_run"],
        default="post_run",
    )
    ...
    if args.phase == "baseline_guard":
        ...
        return 0
    if args.phase == "short800_1000_gate":
        return short800_1000_gate(out)
