...
def main() -> int:
    ...
    rec_v1_override = sum(
        ...
        if str(c.get("decision", "")) == "commit" and str(c.get("decision_reason", "")) == "gap_aware_v1_override"
    )
    rec_v2_override = sum(
        1
        for c in control_events
        if str(c.get("decision", "")) == "commit"
        and str(c.get("decision_reason", "")) == "gap_aware_v2_source_gap_override"
    )
    rec_override_total = rec_v1_override + rec_v2_override
    ...
    stability = {
        ...
        "recovery_commit_override_count": rec_override_total,
        "recovery_commit_v1_override_count": rec_v1_override,
        "recovery_commit_v2_override_count": rec_v2_override,
    ...
    write_csv(
        out_dir / "recovery_commit_control_trace.csv",
        control_events,
    ...
    write_json(
        out_dir / "recovery_commit_control_summary.json",
        {
            ...
            "decision_reason_counts": {
                "normal_commit": sum(1 for c in control_events if str(c.get("decision_reason", "")) == "normal_commit"),
                "gap_aware_v1_override": rec_v1_override,
                "gap_aware_v2_source_gap_override": rec_v2_override,
                "hold_window_rate_limit": sum(
                    1 for c in control_events if str(c.get("decision_reason", "")) == "hold_window_rate_limit"
                ),
                "hold_density_high": sum(
                    1 for c in control_events if str(c.get("decision_reason", "")) == "hold_density_high"
                ),
                "reject_age": sum(1 for c in control_events if str(c.get("decision_reason", "")) == "reject_age"),
                "reject_retry_limit": sum(
                    1 for c in control_events if str(c.get("decision_reason", "")) == "reject_retry_limit"
                ),
            },
        },
    )
    ...
    write_csv(out_dir / "keyframe_timeline.csv", key_rows, list(key_rows[0].keys()) if key_rows else ["frame_id"])
    write_csv(out_dir / "main_chain_gap_timeline.csv", gap_rows, list(gap_rows[0].keys()) if gap_rows else ["from_tick"])
    ...
    report_lines = [
        ...
        f"- recovery_commit_allowed/held/rejected/override: {rec_allowed}/{rec_held}/{rec_rejected}/{rec_override_total}",
        f"- recovery_commit_v1/v2_override_count: {rec_v1_override}/{rec_v2_override}",
