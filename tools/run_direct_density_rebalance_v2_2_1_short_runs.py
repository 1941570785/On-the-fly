#!/usr/bin/env python3
"""Launch baseline guard + v2.2.1 gap-tail short500/800/1000 live trains."""
...
OUT = REPO / "results/StaticHikes/forest1/PAPER_ALIGNED_DIRECT_DENSITY_REBALANCE_V2_2_GAP_TAIL_REFINEMENT_V1"
LIFECYCLE = REPO / "results/StaticHikes/forest1/lifecycle/all_input_frame_lifecycle.csv"
BUILD = REPO / "tools/build_direct_density_rebalance_v2_2_1.py"
...
def main() -> int:
    ...
    for label, nf in (
        ...
        if args.only and args.only not in {key, "all"}:
            continue
        run_dir = OUT / f"direct_density_v2_2_1_{key}"
        jobs.append(
            (
                f"v2_2_1_{key}",
                run_dir,
