...
DATASETS = [
    ...
    ("counter", "datasets/MipNeRF360/counter", -1),
    ("garden", "datasets/MipNeRF360/garden", -1),
    ("forest1", "datasets/StaticHikes/forest1", -1),
    ("forest2", "datasets/StaticHikes/forest2", -1),
    ("university2", "datasets/StaticHikes/university2", -1),
    ("desk1", "datasets/TUM/desk1", -1),
    ("desk2", "datasets/TUM/desk2", -1),
    ("long_office_household", "datasets/TUM/long_office_household", -1),
]
...
FIELDNAMES = [
    ...
    "viewpoint_event_count",
    "viewpoint_rot_last_mean",
    ...
    "anchor_health_score_mean",
    "new_view_event_score_mean",
    "new_view_event_score_p90",
    "low_coverage_count",
    "high_support_concentration_count",
    "high_new_view_event_count",
    "branch",
    "commit",
]
...
def trace_stats(model_dir: Path) -> dict[str, Any]:
    ...
    viewpoint_events = trace.get("viewpoint_coverage_events", [])
    marginal: list[float] = []
    pressure: list[float] = []
    for event in events:
        try:
    ...
    new_view = _float_values(viewpoint_events, "new_view_event_score")
    row = {
    ...
    row.update(
        {
            "viewpoint_event_count": len(viewpoint_events),
            "viewpoint_rot_last_mean": _mean(rot_last),
            ...
            "anchor_health_score_mean": _mean(anchor_health),
            "new_view_event_score_mean": _mean(new_view),
            "new_view_event_score_p90": _quantile(new_view, 0.9),
            "low_coverage_count": sum(value < 0.35 for value in coverage),
            "high_support_concentration_count": sum(value > 0.70 for value in concentration),
            "high_new_view_event_count": sum(value > 0.60 for value in new_view),
        }
    )
    return row
